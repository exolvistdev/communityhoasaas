...
/**
 * Shared page-header: an optional "← back" link, a title (+ optional
 * description) on the left, an optional action on the right. Stacks vertically
 * on mobile (action drops below the title as a full-width button); side-by-side
 * from `sm` up.
 *
  ...
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div className="min-w-0">
        ...
        <h1
          className={`text-lg font-semibold text-fg${backLink ? " mt-1" : ""}`}
        >
          {title}
        </h1>
        {description && (
