...
export async function GET(request: Request) {
  ...
  const rows: (string | number)[][] = [
    ["vendor", "owed", "not_due", "1-30", "31-60", "61-90", "90+"],
  ];
  for (const v of data.vendors)
    rows.push([
      v.vendorName,
      v.outstanding.toFixed(2),
      v.aging.current.toFixed(2),
      v.aging.d1_30.toFixed(2),
      v.aging.d31_60.toFixed(2),
      v.aging.d61_90.toFixed(2),
      v.aging.d90plus.toFixed(2),
    ]);
  rows.push([
    "TOTAL",
  ...
  return csvResponse(
    toCsvString(rows),
    `payables-${org.subdomain}-${range.toYmd}.csv`
  );
}
