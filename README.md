...
| Board elections (RA 9904) — multi-seat "vote for up to N" trustee elections: staff enter a candidate slate, members endorse up to N candidates per unit, top N win (ties for the last seat flagged for a runoff). Finalizing seats the winners as trustees (term dates, officer positions) and can bump their role to Board Member. A configurable "months in arrears" setting (`electionArrearsMonths`) suspends a delinquent unit from voting or running — on elections *and* resolution votes. | ✅ (elections + tally + finalize; portal voting & the Board roster page land in the next slices) |
